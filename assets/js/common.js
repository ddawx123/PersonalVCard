...
$(document).ready(function () {
    ...
    let contextClass = window.AudioContext = window.AudioContext || window.webkitAudioContext || window.mozAudioContext || window.msAudioContext;
    try {
        ...
        //这里用来存储背景音乐的路径
        try {
            loadAudioFile('assets/music/551400874.mp3');
        } catch (e) {
            try {
                loadAudioFile('https://music.163.com/song/media/outer/url?id=551400874.mp3');
            } catch (e) {
                console.log('本地和远程背景音乐装入过程均出现异常，错误信息：', e);
            }
        }
    } catch (e) {
        console.log('无法找到音乐！');
